const passwords = `13-14 f: ffffffffnfffvv
...
const findBadPasswords = (passString) => {
  let passArr = passString.split('\n');
  let validPasswordCount = 0;
  for (let i = 0; i < passArr.length; i++) {
    letterCount = 0;
    let position1 = [];
    let position1Found = false;
    let position2 = [];
    let position2Found = false;
    let validatingLetter = '';

    password = [];

    for (let j = 0; j < passArr[i].length; j++) {
      let currentChar = passArr[i][j];


      if (!position1Found) {
        if (currentChar === '-') {
          position1Found = !position1Found;
        } else {
          position1.push(currentChar)
        }
      } else if (!position2Found) {
        if (currentChar === ' ') {
          position2Found = !position2Found;
          validatingLetter = passArr[i][j + 1];
          j += 2;
        } else {
          position2.push(currentChar);
        }
      } else {
        password = passArr[i].slice(++j);
        break;
      }
    }
    position1 = parseInt(position1.join(''));
    position2 = parseInt(position2.join(''));

    if (password[position1 - 1] !== password[position2 - 1] && (password[position1 - 1] === validatingLetter || password[position2 - 1] === validatingLetter)) {
      validPasswordCount++
    }
  }
...
console.log(findBadPasswords(passwords));
