...
const findHighestSeatID = (allSeats) => {
  ...
  for (let i = 0; i < seatsArr.length; i++) {
    ...
    for (let j = 0; j < seatsArr[i].length; j++) {
      let currentLetter = seatsArr[i][j];
      if (currentLetter === 'F') {
...
/*
O: the highest value seat on the airplane
I: A string of all used seats on the plane
C: x
E: x

The generate plan is some slight optimization by storing whether consecutive Fs were found - this should convert things to n log n average. Worst case is if our major consecutive Fs are at the end ofh the iteration
